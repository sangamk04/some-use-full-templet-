import React from 'react'
// import "./score.css"
import{Box} from "@chakra-ui/react";

import {
  Table,
  Thead,
  Tbody,
  Tfoot,
  Tr,
  Th,
  Td,
 TableContainer,
} from '@chakra-ui/react'

const Scoredescription = () => {
  return (<div >
 <div className="Main">
 <Box  >
    <Box border='1 px solid #dfdfe1' borderRadius='lg' w='56.4%' marginLeft={"180px"} marginTop={""}  backgroundColor="white">
    <TableContainer>
  <Table variant='' colorScheme='teal'>
    
    <Thead>
      <Tr>
        <Th fontSize={"1xl"} borderBottom="1px">INTERNATIONAL TOUR</Th>
        {/* <Th fontSize={"1xl"}>|</Th> */}
        <div></div>
        <Th fontSize={"1xl"} borderBottom="1px">ICC TOURNAMENTS</Th>
      </Tr>
    </Thead>
    <Tbody>
      <Tr>
      <Td color={"Highlight"}>England v South Africa</Td>
        <Td></Td>
        <Td color={"Highlight"}>ICC Women's Championship</Td>
      </Tr>
      <Tr>
      <Td color={"Highlight"}>Australia v Zimbabwe</Td>
        <Td></Td>
        <Td color={"Highlight"}>Men's Cricket World Cup League 2</Td>
      </Tr>
      <Tr>
      <Td color={"Highlight"}>NZ-A in India</Td>
        <Td></Td>
        <Td color={"Highlight"}>CWC Challenge League Group A</Td>
      </Tr>
      <Tr>
      <Td color={"Highlight"}>SL U-19s in ENG</Td>
        <Td></Td>
        <Td color={"Highlight"}>CWC Challenge League Group B</Td>
      
      </Tr>
      <Tr>
      <Td color={"Highlight"}>IRE-W in NL</Td>
        <Td></Td>
        <Td color={"Highlight"}>ICC CWC Super League</Td>
      </Tr>
    </Tbody>
    <Tfoot>
      <Tr>
      <Th fontSize={"1xl"} borderBottom="1px">INTERNATIONAL TOURNAMENTS</Th>
        <Th></Th>
        <Th fontSize={"1xl"} borderBottom="1px">T20/T10 TOURNAMENTS</Th>
      </Tr>
      <Tr>
      <Td color={"Highlight"}>Asia Cup Qualifier</Td>
        <Td></Td>
        <Td color={"Highlight"}>Asia Cup 2022</Td>
      
      </Tr>
      <Tr>
      <Td color={"Highlight"}>Asia Cup 2022</Td>
        <Td></Td>
        <Td color={"Highlight"}>SLC T20</Td>
      
      </Tr>
      <Tr>
      <Td color={"Highlight"}>CWC Challenge Group A</Td>
        <Td></Td>
        <Td color={"Highlight"}>Maharaja T20</Td>
      
      </Tr>
      <Tr>
      <Td color={"Highlight"}>CWC Challenge Group B</Td>
        <Td></Td>
        <Td color={"Highlight"}>6IXTY (W)</Td>
      
      </Tr>
      <Tr>
      <Td color={"Highlight"}>ICC World Test Championship</Td>
        <Td></Td>
        <Td color={"Highlight"}>6IXTY (M)</Td>
    </Tr>
     
      <Tr>
      <Th ></Th>
        <Th></Th>
        <Th fontSize={"1xl"} borderBottom="1px">ASSOCIATE CRICKET</Th>
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>Kenya v Nepal</Td>
      
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>Bahrain v Kuwait</Td>
      
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>ITA-W in Austria</Td>
      
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>Continental Cup</Td>
      
      </Tr>

       
    
      <Tr>
      <Th ></Th>
        <Th></Th>
        <Th fontSize={"1xl"} borderBottom="1px">AUSTRALIA DOMESTIC</Th>
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>Sheffield Shield</Td>
      
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>The Marsh Cup</Td>
      
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>BBL 2022 </Td>
      
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>WBBL 2022</Td>
      
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>RL Cup</Td>
      
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>WNC League</Td>
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}></Td>
      
      </Tr>


      <Tr>
      <Th ></Th>
        <Th></Th>
        <Th fontSize={"1xl"} borderBottom="1px">ENGLAND DOMESTIC</Th>
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>County Div1</Td>
      
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>County Div2</Td>
      
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>100  (Men's) </Td>
      
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>100 (Women's)</Td>
      
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>RL Cup</Td>
      
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>Vitality Blast</Td>
      
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>RHF CUP</Td>
      
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}></Td>
      
      </Tr>
      <Tr>
      <Th ></Th>
        <Th></Th>
        <Th fontSize={"1xl"} borderBottom="1px">NEW ZEALAND DOMESTIC</Th>
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>Plunket Shield</Td>
      
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>Ford Trophy</Td>
      
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>Women's 1-Day</Td>
      
      </Tr>
      <Tr>
      <Th ></Th>
        <Th></Th>
        <Th fontSize={"1xl"} borderBottom="1px">WEST INDIES DOMESTIC</Th>
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>6IXTY (W)</Td>
      
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>6IXTY (M)</Td>
      
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>CPL 2022</Td>
      
      </Tr>
      <Tr>
      <Td ></Td>
        <Td></Td>
        <Td color={"Highlight"}>WCPL 2022</Td>
      
      </Tr>
    </Tfoot>
  </Table>
</TableContainer>
</Box>
</Box>

 </div>
</div>
  )
}
export default Scoredescription